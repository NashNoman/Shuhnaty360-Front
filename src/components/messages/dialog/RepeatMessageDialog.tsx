...
const RepeatMessageDialog = ({ isDialogVisible, setIsDialogVisible }: any) => {
  ...
  return (
    <BootstrapDialog onClose={handleClose} aria-labelledby='customized-dialog-title' open={isDialogVisible}>
      <DialogTitle
        sx={{ m: 0, p: 2 }}
      ...
      <DialogActions sx={{ margin: '10px 6px 0' }}>
        <div className='grid grid-cols-2 gap-4 w-full mb-2'>
          <button className='col-span-1 bg-[#DD7E1F] text-[#FCFCFC] border border-[#DD7E1F] py-3 rounded-lg'>تطبيق</button>
          <button className='col-span-1 bg-[#FCFCFC] text-[#DD7E1F] border border-[#DD7E1F] py-3 rounded-lg'>حذف</button>
        </div>
      </DialogActions>
    </BootstrapDialog>
  );
