...
const UserDriverDetailsTable = ({
  ...
  const filteredData = userShipmentsData.filter(
    (shipment: any) => selectedOption === 'all' || shipment.date === selectedOption,
  );
  ...
  const tableHeading = ['الرقم', 'المصدر', 'الوجهة', 'رقم الشحنة', 'تاريخ التحميل'];
          ...
          {paginatedData.map((shipment: any, index: any) => (
            <>
              <tr className={`rounded-lg ${index % 2 === 0 && 'bg-[#F2F2F2]'}`}>
                ...
                >
                  <td className={tableRowStyles}>{(currentPage - 1) * itemsPerPage + index + 1}</td>
