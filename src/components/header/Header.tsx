...
const PAGE_TITLES = [
  ...
  { path: '/alert-messages/select-recipients', title: 'تحديد مستلمين', exact: true },
];
...
const Header = () => {
  ...
  const getPageTitle = () => {
    ...
    const exactMatch = PAGE_TITLES.find(item => 
      item.exact && pathname === item.path
    );
    if (exactMatch) return exactMatch.title;

    const partialMatch = PAGE_TITLES.find(item => 
    ...
    <header className='flex justify-between items-center mt-16 lg:mt-12 mb-10 px-8 gap-4'>
      ...
      {showBackButton ? (
        <button
          ...
          aria-label={pathname.includes('/shipments/delete-shipment') ? 'إلغاء' : 'عودة'}
        >
        ...
        <div className='flex gap-2 items-center'>
          <button className='xs:hidden p-2 rounded-full bg-[#E6E6E6]' aria-label="الإشعارات">
