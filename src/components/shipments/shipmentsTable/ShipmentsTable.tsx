...
const ShipmentsTable = ({
  ...
  const [loadingAtSort, setLoadingAtSort] = React.useState<
    "asc" | "desc" | undefined
  >();
  ...
  filterableColumns.forEach((col) => {
    ...
    if (col.key === "loading_at") {
      values = values
        .filter(Boolean)
        .map((dateStr: string) => formatDate(dateStr));
    }
    ...
    (shipment: any) =>
      Object.keys(filters).every((key) => {
        ...
        return (
          ...
          filters[key].includes(shipment[key])
        );
      }) &&
  ...
  if (loadingAtSort === "asc") {
    sortedData.sort(
      (a, b) =>
        new Date(a.loading_at || 0).getTime() -
        new Date(b.loading_at || 0).getTime(),
    );
  }
