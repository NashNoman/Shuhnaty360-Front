...
function getDeliveredShipmentsChartData(deliveredShipments: any[], range: string) {
...
function getFilteredDeliveredShipments(deliveredShipments: any[], range: string) {
  const { start, end } = getRangeDates(range);
  return deliveredShipments.filter((shipment: any) => {
    const date = parseShipmentDate(shipment.expected_arrival_date || shipment.actual_delivery_date);
...
const Dashboard = React.memo(() => {
  ...
  const returnedShipments = shipments.filter((shipment: any) => shipment.status === 'مرتجعة');
  ...
  const delayedShipments = shipments.filter((shipment: any) => shipment.status === 'متأخرة');
  ...
  const percentage =
    shipments.length === 0 ? 0 : ((filteredDelivered.length / shipments.length) * 100).toFixed(1);
  ...
  const shipmentsStatus = [
    ...
    },
    {
      ...
      styles: 'hover:bg-[#CA8B02] text-[#CA8B02] font-bold hover:text-[#FCFCFC]',
    },
    {
  ...
  const pieCounts = pieStatuses.map(
    (item) => filteredShipments.filter((shipment: any) => shipment.status === item.status).length,
  );
  ...
  const series = useMemo(() => {
    ...
    return branchStatuses.map((status) => ({
      ...
      data: uniqueCities.map(
        (city) =>
          filteredShipments.filter((s) => s.origin_city === city && s.status === status.name).length,
      ),
    }));
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-2 '>
          <div className='p-4 col-span-1 lg:col-span-2 border border-gray-200 shadow-md rounded-3xl '>
            <div className='flex flex-col lg:flex-row justify-between mb-10'>
              ...
              <div
                className={`mt-12 lg:mt-6 flex items-center rounded-2xl border gap-6 md:gap-12 lg:gap-16 bg-[#F2F2F2] h-fit w-fit px-4 m-auto lg:m-0 max-w-full ${
                  selectedRange === 'أسبوعي' ? 'ps-0' : selectedRange === 'سنوي' ? 'pe-0' : ''
                }`}
              >
                {['أسبوعي', 'شهري', 'سنوي'].map((item, index) => (
                  <button
                    ...
                    className={`font-Rubik transition-all duration-300 ${
                      selectedRange === item && 'bg-[#DD7E1F] text-[#FCFCFC] py-3 px-6 rounded-2xl'
                    }`}
            ...
            <div className='w-full flex justify-center items-center'>
              {' '}
              <span>
                الفترة: {start && end ? `${formatDate(start)} - ${formatDate(end)}` : 'كل الوقت'}
              </span>
            </div>
          ...
          <div className='col-span-1 shadow-md rounded-3xl'>
            <div className='flex items-center justify-center gap-2 my-12'>
              <h1 className='xs:text-lg text-xl font-bold text-[#333333]'>تقرير الشحنات</h1>
              <span className='xs:text-xs text-sm text-[#666666] font-Rubik'>{selectedRange}</span>
            </div>
            <PieChart data={pieChartData} />
          </div>
        </div>
        <div className={`mt-8 shadow-md rounded-3xl ${isSidebarOpen && 'flex flex-col shrink'}`}>
          <div className='flex justify-between items-center p-4'>
            <div className='flex flex-col gap-2'>
              <span className='text-[#666666] xs:text-xs text-sm font-Rubik'>{selectedRange}</span>
              <span className='xs:text-lg text-xl sm:text-2xl text-[#333333] font-bold'>
                تقرير الفروع
              </span>
            </div>
            <div className='flex flex-col items-center gap-2'>
              <span className='text-[#666666] xs:text-xs text-sm font-Rubik'>الإجمالي:</span>
              <span className='xs:text-lg text-xl sm:text-2xl text-[#333333] font-bold'>{totalShipments} شحنة</span>
            </div>
          </div>
          <HorizontalChart
            series={series}
